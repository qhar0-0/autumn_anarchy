...
@onready var a = 0
@onready var return_button_asset = preload("res://return_button.tscn")
...
func _process(_delta: float) -> void:
	...
	anim_wings.position.y + (sin(a/40) * .1))
	...
	if Game.lives <= 0:
		...
		if !rbutton_exists:
			return_button = return_button_asset.instantiate()
		...
		moveable = false
		get_node("../hurter_spawner").spawnable = false
	else:
		rbutton_exists = 0
		if return_button != -1:
